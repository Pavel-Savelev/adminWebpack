...
export type SecondaryContentType = "map" | "logs" | "updates" | "photos" | "another" | "accidents" | "comments" | "availability" | "monitoring";
...
function StationDetails() {
  ...
  useEffect(() => {
    if ((location.state as LocationState)?.station) {
      ...
      const found = allStations.find(s => String(s.productNumber) === paramProductNumber);
  ...
  const filterLogsStation = useMemo(() => 
    logSocketList.filter(log => log.productNumber === station?.productNumber),
